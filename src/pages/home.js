...
import { Modal, NewsCard } from "../components";
...
export const Home = () => {
  ...
  const handleShare = (uuid) => {
    const url = `${window.location.href}/details/${uuid}`;
    setShareUrl(url);
    setModalIsOpen(true);
  };
  ...
  return (
    <Box positon="relative">
      <Modal url={shareUrl} open={modalIsOpen} handleClose={handleCloseModal} />
      <Grid
        container
        ...
        {pagData?.map((item) => {
          return (
            <Grid item key={item.url}>
              <NewsCard data={item} onShare={() => handleShare(item.uuid)} />
            </Grid>
          );
        })}
      </Grid>
